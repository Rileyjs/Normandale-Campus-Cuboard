VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} Reentry 
   Caption         =   "Data Rentry"
   ClientHeight    =   4335
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   6975
   OleObjectBlob   =   "Reentry.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "Reentry"
...
For entryCounter = 1 To itemCount
   ' Fills A and B with date and techID (textbox1.value). The first line is the first of the two differencesbetween this UserForm
   ' and UserForm1. This version takes the date value provided by the user in Textbox3.
    Worksheets(Sheets.Count).Range("A" & currentRows).Value = Format(TextBox3.Value, "Short Date")
    ...
    ' Fills in Column D and E with finacial need box (ComboBox7.Value) and time. The second line is the second of the two differences
    ' between this UserForm and UserForm1. This version takes the time value provided by the user in Textbox2.
    Worksheets(Sheets.Count).Range("D" & currentRows).Value = ComboBox7.Value
    Worksheets(Sheets.Count).Range("E" & currentRows).Value = Format(TextBox2.Value, "h:mm") + " " + ComboBox8.Value
